...
class VideoProcessingUnitKafkaResourceIT {
    ...
    void setup() {
        ...
        VideoProcessingUnitKafkaResource kafkaResource = new VideoProcessingUnitKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/video-processing-unit-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/video-processing-unit-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
